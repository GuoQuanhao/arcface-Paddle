...
class CallBackVerification(object):
    ...
    def ver_test(self, backbone: paddle.nn.Layer, global_step: int, batch_size: int):
        results = []
...
class CallBackLogging(object):
    ...
    def __call__(self, global_step, loss: AverageMeter, epoch: int, fp16: bool, grad_scaler: paddle.amp.GradScaler):
...
class CallBackModelCheckpoint(object):
    ...
    def __call__(self, global_step, backbone: paddle.nn.Layer, partial_fc: PartialFC = None):
        if global_step > 100 and self.rank is 0:
            paddle.save(backbone.state_dict(), os.path.join(self.output, "backbone.pdparams"))
