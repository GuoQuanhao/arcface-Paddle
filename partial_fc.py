...
class PartialFC(nn.Layer):
    ...
    def __init__(self, rank, local_rank, world_size, batch_size, resume,
        ...
        self.weight_name = os.path.join(self.prefix, "rank:{}_softmax_weight.pkl".format(self.rank))
        self.weight_mom_name = os.path.join(self.prefix, "rank:{}_softmax_weight_mom.pkl".format(self.rank))
        ...
        if resume:
            try:
                ...
                self.weight = paddle.normal(0, 0.01, (self.num_local, self.embedding_size))
                print("softmax weight resume fail!")

            try:
                self.weight_mom: paddle.Tensor = paddle.load(self.weight_mom_name)
        ...
        if int(self.sample_rate) == 1:
            ...
            self.sub_weight = paddle.create_parameter(shape=self.weight.shape, dtype='float32', default_initializer=paddle.nn.initializer.Assign(self.weight))
            self.sub_weight_mom = self.weight_mom
        else:
            self.sub_weight = paddle.create_parameter(shape=[1, 1], dtype='float32', default_initializer=paddle.nn.initializer.Assign(paddle.empty((1, 1))))
    ...
    def sample(self, total_label):
        index_positive = (self.class_start <= total_label).numpy() & (total_label < self.class_start + self.num_local).numpy()
    ...
    def forward_backward(self, label, features, optimizer):
        ...
        with paddle.no_grad():
            ...
            # get one-hot
            grad = logits_exp
            one_hot = paddle.nn.functional.one_hot(total_label.astype('long'), num_classes=85742)

            # calculate loss
            loss = paddle.nn.functional.one_hot(total_label.astype('long'), num_classes=85742).multiply(grad).sum(axis=1)
            loss_v = paddle.clip(loss, 1e-30).log().mean() * (-1)

            # calculate grad
            grad -= one_hot
            grad = grad.divide(paddle.to_tensor(self.batch_size * self.world_size, dtype='float32'))
